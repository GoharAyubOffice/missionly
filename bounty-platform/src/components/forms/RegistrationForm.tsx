...
interface RegistrationFormProps {
  ...
  isLoading?: boolean;
  onGoogleSignUp?: (role: 'CLIENT' | 'FREELANCER') => Promise<void>;
}

export function RegistrationForm({ onSubmit, isLoading = false, onGoogleSignUp }: RegistrationFormProps) {
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Button
              type="button"
              variant="secondary"
              onClick={() => onGoogleSignUp('FREELANCER')}
              disabled={isLoading || isSubmitting}
              className="w-full"
            >
              <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
